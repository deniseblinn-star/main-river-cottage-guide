...
import { recipeAssignmentCount } from '../utils/eventGroceryEngine'
...
export default function Recipes(){
 ...
 const remove=(recipe)=>{
  if(recipeAssignmentCount(activeEvent,recipe.id)>0){
 ...
 return <div className="space-y-4">
  ...
  <div className="grid sm:grid-cols-2 gap-3">{list.map(r=>{
   const assignmentCount=recipeAssignmentCount(activeEvent,r.id)
   const assigned=assignmentCount>0
   return <article key={r.id} className="card card-hover">
    <Link to={`/recipes/${r.id}`} className="block">
     <div className="flex justify-between gap-3"><div className="text-3xl">{emoji[r.method]||'🍽️'}</div>{assigned?<span className="badge bg-forest/10 text-forest h-fit">Assigned {assignmentCount>1?`(${assignmentCount})`:''}</span>:<span className="badge bg-stone/10 text-stone h-fit">Unassigned</span>}</div>
